...
void* packet_consumer_thread(void *_id) {
  ...
  while (!do_shutdown) {
    if (!enable_tx) {
      ...
      //如果打开了tx口，则调用recv_pkt_buff函数。。。
      //typedef void pfring_pkt_buff;，这货是个void指针
  ...
  if (bridge_interfaces) {
	  for (i = 0; i < num_dev; i++) {
	    if (if_indexes[i] == hdr.extended_hdr.if_index) {
	      ...
	      break;
	    }//如果if_indexes[i]==该包的hdr中扩展hdr的if接口的id时，设置pfring为...
	  }//End of for loop....
    ...
    }//End of if (!enable_tx) 's Else branch....
